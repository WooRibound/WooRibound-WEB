...
export default {
  ...
  setup() {
    ...
    const onApprovalClick = (entId) => {
      ...
      modalMessage.value = "회원탈퇴를 승인하겠습니까?";
    ...
    const onRejectionClick = (entId) => {
      ...
      selectedEntId.value = entId;
      modalMessage.value = "회원탈퇴를 반려하겠습니까?";
      isModalVisible.value = true;
    };
      ...
      <table class="applicant-table">
        ...
        <tbody>
          <tr v-for="corporateUser in corporateUserList" :key="corporateUser.entId">
            <td>
              {{ formatDate2(new Date(Date.parse(corporateUser.deleteRequestedAt))) }}
              <br>
              {{ new Date(corporateUser.deleteRequestedAt).toLocaleTimeString('ko-KR', {
                hour: '2-digit',
                minute: '2-digit',
                hour12: false
              }) }}
